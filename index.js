// (() => {
    ...
    const clickEvent = () => {
        ...
        // console.log('_scene.pointerX',_scene.pointerX);
        // console.log('_scene.pointerY',_scene.pointerY);
        var pickResult = _scene.pick(_scene.pointerX * dpi, _scene.pointerY * dpi);


        // if we clicked on the mesh then change its color
        if (pickResult.hit === true) {
            // look at the result in console log
    ...
    // const swapCubes = (index_1, index_2) => {
    ...
    //     let swapString = "position.x";

    //     if (Math.round(startx1) === Math.round(startx2)) {
    ...
    const rotateCubeUp = (p_cube) => {
        ...
        BABYLON.Animation.CreateAndStartAnimation("rotateUp", p_cube, "rotation.x", 240, 60, start, end, 0, BABYLON.EasingFunction.easeInOutCubic, () => {
            p_cube.position = new BABYLON.Vector3.Zero();
            p_cube.bakeCurrentTransformIntoVertices();
            p_cube.position = startPos;
            p_cube.rotation = new BABYLON.Vector3.Zero();
            restoreVertexData(p_cube);
            _animating--;
            _solved = checkCompletion();
        });
