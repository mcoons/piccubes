// (() => {
        ...
        for (let row = _rows, boxID = 0; row > 0; row--)
            for (let column = 0; column < _columns; column++, boxID++) {
                ...
                _cube[boxID].userData = {
                    originalIndex: boxID,
                    globalRotationTransform: new BABYLON.TransformNode("cube"+boxID)
                };
    ...
    const createScene = () => {
        ...
        // var id = setTimeout(() => {
        //     // console.log("swapping cubes");
        //     rotateCube(_cube[3], "rotation.x", 1);
        // }, 1000);
    ...
    const checkCompletion = () => {
        let result = true;
        let rx = _cube[0].userData.globalRotationTransform.rotation.x;
        let ry = _cube[0].userData.globalRotationTransform.rotation.y;
        let rz = _cube[0].userData.globalRotationTransform.rotation.z;

        _cube.forEach( (c,i) => {
            if (c.userData.globalRotationTransform.rotation.x != rx || c.userData.globalRotationTransform.rotation.y != ry || c.userData.originalIndex != i)
                result = false;
        });
    ...
    const rotateCube = (p_cube, p_axis, p_dir) => { // right/down is -,  "rotation.x", "rotation.y"
        ...
        const gStart = p_axis === "rotation.x" ?  p_cube.userData.globalRotationTransform.rotation.x : p_cube.userData.globalRotationTransform.rotation.y;
        const gEnd = gStart + p_dir*Math.PI / 2;

        BABYLON.Animation.CreateAndStartAnimation("rotation", p_cube.userData.globalRotationTransform, p_axis, 60, 5, gStart, gEnd, 0, BABYLON.EasingFunction.easeInOutCubic, () => {
            p_cube.userData.globalRotationTransform.rotation.x = p_cube.userData.globalRotationTransform.rotation.x % (2*Math.PI);
            p_cube.userData.globalRotationTransform.rotation.y = p_cube.userData.globalRotationTransform.rotation.y % (2*Math.PI);
        });
        BABYLON.Animation.CreateAndStartAnimation("rotation", _selector, p_axis, 60, 15, start, end, 0, BABYLON.EasingFunction.easeInOutCubic, () => _selector.rotation = new BABYLON.Vector3.Zero());
        BABYLON.Animation.CreateAndStartAnimation("rotation", p_cube, p_axis, 60, 15, start, end, 0, BABYLON.EasingFunction.easeInOutCubic, () => {
